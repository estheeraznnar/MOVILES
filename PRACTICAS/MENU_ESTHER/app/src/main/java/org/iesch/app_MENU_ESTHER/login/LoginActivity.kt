...
<<<<<<< HEAD
    ...
    private fun verificarDatosGuardados() {
        lifecycleScope.launch {
            ...
            val isLogged = loginDataStore.isLogger.first()


            //Hago un toast para verificarlo visualmente
            if (email.isNotEmpty()) {
                Toast.makeText(this@LoginActivity, "Usuario guardado: $email\nlogueado: ${if (isLogged) "Si" else "No"}", Toast.LENGTH_LONG).show()
            } else {
                Toast.makeText(this@LoginActivity, "No hay usuarios registrados", Toast.LENGTH_LONG).show()
            }
        }
    }
